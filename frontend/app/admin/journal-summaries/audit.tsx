...
export default function SummaryAuditPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError('');
      try {
        const data = await getSummaryAuditTrail(params.id, token);
        setLogs(data);
      } catch {
  ...
  return (
    <div className="flex flex-col items-center min-h-screen p-4 space-y-4">
      <Link href={`/admin/journal-summaries/${params.id}`} className="self-start text-blue-600 underline">
        Back
      </Link>
      <div className="flex gap-4">
        <Link href={`/admin/journal-summaries/${params.id}`}>Summary</Link>
        <Link href={`/admin/journal-summaries/${params.id}/override`}>Notes</Link>
